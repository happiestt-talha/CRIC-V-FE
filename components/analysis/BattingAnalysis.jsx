  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Card className="bg-slate-900 border-slate-800 p-6 overflow-hidden relative">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
          <div className="space-y-1">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Player Stance</p>
            <h2 className="text-3xl font-black text-slate-100">{stance_type || 'N/A'}</h2>
          </div>
          <div className="space-y-1 md:text-right">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Shot Selection</p>
            <Badge className={cn(
              "px-3 py-1 text-sm font-black uppercase",
              shot_selection ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-slate-800 text-slate-500"
            )}>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest">Weight Distribution</h3>
        {weight_distribution ? (
          <div className="h-12 bg-slate-800/50 rounded-xl p-1 flex items-stretch ring-1 ring-slate-700/50">
            <div 
              className="bg-blue-600 rounded-l-lg flex items-center justify-center text-[10px] font-black text-white"
              style={{ width: `${weight_distribution.left || 50}%` }}
            >
              FRONT: {weight_distribution.left || 50}%
            </div>
            <div 
              className="bg-slate-700 rounded-r-lg flex items-center justify-center text-[10px] font-black text-slate-300"
          ...
          <div className="h-12 bg-slate-800/30 rounded-xl flex items-center justify-center border border-dashed border-slate-700">
            <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">N/A — DATA UNAVAILABLE</span>
          </div>
        )}
      </div>
